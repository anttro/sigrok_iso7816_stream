...
def plausible_cla(cla):
    '''Check if CLA byte is a structurally valid ISO 7816 class.

    Per ISO 7816-4, the CLA byte is laid out as:

      b8 b7 b6 : interindustry / proprietary class
      bits 4-3 : secure messaging indicator
      bits 2-1 : logical channel number (0-3 for interindustry)

    Interindustry CLA high nibble is 0x0/0x1/0x4/0x5 (i.e. CLA & 0xF0 in
    (0x00, 0x10, 0x40, 0x50)); this covers 0x00 (basic), 0x01-0x03
    (logical channels 1-3, e.g. the command that follows a MANAGE CHANNEL
    that opened channel 1), 0x0C (secure messaging), chaining variants,
    and the structural/SM classes 0x4x/0x5x.

    b8 = 1 marks proprietary structural formats; only the classes actually
    used by SIM/UICC CAT stacks (3GPP TS 102 221 / TS 51.011) are accepted.

    Anything else means the decoder likely mis-framed the byte stream and
    should attempt to resync.'''
    if not (cla & 0x80):
...
def validate_t0_apdu(packet):
    ...
    APDUs (several real exchanges swallowed into one "valid" packet) pass
    this structural check; they are caught separately by
    embedded_exchanges() and flagged CONCAT/BAD_FCS at emit time.
    '''
    n = len(packet)
...
def embedded_exchanges(buf):
    ...
    valid (same rule as plausible_cla); INS must be a real command byte;
...
class Decoder(srd.Decoder):
    ...
    def _note_discard(self, value, reason, idle=False):
        # Report bytes consumed but not emitted as part of an APDU.  Idle
        # line noise (0x00/0xFF on an idle period) is counted and reported
        # periodically so a long idle line cannot flood the session log;
        # every other discard (real data) is logged and annotated always.
        if idle:
            self._idle_discard_total += 1
            if self._idle_discard_total == 1 or self._idle_discard_total % 256 == 0:
                self.log("discarded idle byte 0x%02x: %s (idle discards so far: %d)"
                         % (value, reason, self._idle_discard_total))
            return
        self._discard_total += 1
        self.log("discarded byte 0x%02x: %s (discards so far: %d)"
                 % (value, reason, self._discard_total))
        if self.ss is not None and self.ss >= 0:
    ...
    def _consume_first(self):
        ...
        whichever source it came from (the _replay buffer or the peeked
        byte).  The decode-step consume paths use this so re-framed bytes
        queued in _replay cannot loop forever.'''
        if self._replay:
    ...
    def reset(self):
        ...
        self._stall = 0
        # One-shot: at the first decode_step the initial RST/VCC levels are
        # primed from the real first sample instead of the defaults below.
        # Without this, last_rst=0/last_vcc=0 make "capture started while the
        ...
        self._resync = False
        # Discard accounting: every consumed byte that is not emitted as
        # part of an APDU is reported.  Non-idle discards (real data) are
        # logged + annotated at every occurrence; idle 0x00/0xFF line noise
        # is counted and reported periodically so a long idle line cannot
        # flood the session log.
        self._discard_total = 0
        self._idle_discard_total = 0
    ...
    def start(self):
        ...
        self._pcap_base_time = time.time()
        ...
        # clock_skip is not trusted until recovered (the card may have already
        # PPS'd to a faster ETU before the capture began).
            ...
            if emit_lvl is not None:
                ...
                if (emit_lvl == 1):
                    ...
                    if self._atr_parsed:
                        ...
                        # _atr_parsed is False: a fresh ATR is expected.  The
                        # 10ms hysteresis confirms the deassert ~10ms after
                        # the physical edge -- typically in the MIDDLE of that
                        # very ATR.  By then the hunt is already armed with
                        # the correct default rate (clock_skip=372,
                        # bit_samples=None): either by the RST assert handler
                        # (warm reset, edge observed) or by the "RST low at
                        # start" cold-boot path.  Re-arming here would clear
                        # the in-flight ATR's timing and garble it, so only
                        # re-arm if we are still sitting in DATA with nothing
                        # armed (the one case the assert handler/cold-boot
                        # path never covered: a deassert without a preceding
                        # observed assert or hunt).
                        if self.state == 'DATA':
                            self.state = 'FIND START'
                            self._atr_hunt_count = 0
                            self._pps_attempted = False
                            ...
                            self.clock_skip = 372
                            self._inverse_convention = False
                            self._clock_skip_confident = \
                                not (self.sample_as_clock or self.detect_clock)
                            self.log("RST deasserted with no hunt armed; re-arming ATR hunt")
                        else:
                            self.log("RST deasserted (hunt/ATR already in progress)")
        if (self.track_vcc):
    ...
    def _deglitch_edges(self, edges, bs):
        '''Remove pulses shorter than a fraction of an ETU from an edge list.

        Phone traces (especially Samsung at 16 CLK/ETU) contain 1-sample
        (62.5 ns) LOW glitches in inter-byte guard times.  These are far
        shorter than the shortest real ISO 7816 pulse (about 1 ETU) and
        would otherwise be accepted as false start-bit falls.

        The filter uses an iterative narrowest-first strategy: at each step
        it finds the shortest pulse, and if it is shorter than 0.2 * bs it
        removes the edge that *ends* the pulse.  Removing the end edge lets
        the previous (real) edge's value persist, re-merging the surrounding
        fragments back into the original ETU-multiple pulse.  The first edge
        (the start-bit fall passed to _read_byte_edges) is never removed.

        Returns a new edge list; the input list is not modified.'''
        if not edges:
            return edges
        threshold = max(1, int(0.2 * bs))
        # Work with a mutable copy.  Each item is [sample, value].
        out = [[s, v] for (s, v) in edges]
        changed = True
        while changed:
            changed = False
            # Find the shortest pulse (distance between consecutive edges).
            min_width = None
            min_idx = None
            n = len(out)
            for i in range(n - 1):
                width = out[i + 1][0] - out[i][0]
                if width < threshold and (min_width is None
                                          or width < min_width):
                    min_width = width
                    min_idx = i
            if min_idx is not None:
                # Remove the edge that ends the short pulse.  This keeps the
                # preceding edge (the real transition) and lets its value
                # persist until the next edge.  If the short pulse starts at
                # the first edge, keep the first edge and remove the second.
                del out[min_idx + 1]
                changed = True
        return [(s, v) for [s, v] in out]
    ...
    def _next_start_fall(self, edges, after, bs):
        '''First GENUINE byte-start FALL (1->0) at or after sample `after`,
        = the next byte start.

        A real start bit is LOW for a full ETU.  A sub-ETU fall inside the
        stop-bit / inter-byte guard time is a phantom (line bounce) and must
        not be treated as the next byte's start -- latching onto it shifts
        the whole frame.  A fall is accepted only if the line stays LOW for
        at least half an ETU after it (a genuine start bit's low hold is
        ~1 ETU; the first data-bit transition arrives no sooner).'''
        thresh = int(0.5 * bs)
        for i in range(len(edges)):
            s, v = edges[i]
            if s < after or v != 0:
                continue
            nxt = edges[i + 1][0] if i + 1 < len(edges) else s + thresh + 1
            if (nxt - s) < thresh:
                continue
            return s
        return None
    ...
    def _read_byte_edges(self, bs):
        ...
        # Use a 15 ETU window so a 2 ETU turnaround idle (observed on
        # Samsung phone at 16 CLK/bit) still leaves the next start-bit fall
        # inside the collected edge list.
        edges = [(e0, 0)]
        limit = e0 + int(15 * bs)
        last_sn = e0
        while True:
            ...
            if sn > limit:
                break
        # Remove sub-ETU glitch pulses (common on phone SIMs at fast ETUs
        # like 16 CLK/bit).  Without this the edge reader may latch onto a
        # 1-sample bounce in an inter-byte guard time and read the wrong
        # byte, desynchronising the whole T=0 frame.
        edges = self._deglitch_edges(edges, bs)
        ...
        nxt = None
        if bits is not None and bits[0] == 0:
            self.bits = bits
            byte = self.get_bytes(bits[1:9])
            nxt = self._next_start_fall(edges, e0 + int(10 * bs), bs)
        else:
            # bits[0] is the start bit; it must be 0 after _bits_at() because
            # _bits_at() already inverts the value for inverse convention.
            # bits[0] != 0 means we latched onto a spurious edge (phantom).
            # Do NOT drop the byte: dropping replaces a real byte with a fake
            # 0x00 / shifts the exchange boundary, which is exactly how a
            # whole exchange gets swallowed (CONCAT).  Instead slide forward
            # to the next genuine start-bit fall inside this same edge window
            # and re-anchor there.
            for (s, v) in edges:
                if s <= e0 or v != 0:
                    continue
                cand = self._bits_at(s, edges, bs)
                ...
                # No genuine start in the window (EOF / pathological line):
                # keep the legacy drop; the next read re-hunts.
                self.bits = []
                byte = 0
        self.ss = e0
        if nxt is not None:
    ...
    def wait_data_falling(self):
        ...
        while True:
            ...
            if (self.samplenum - hi_start) < min_high:
                continue
            if self._min_pulse_samples > 0:
                pins = self.wait({'skip': self._min_pulse_samples})
                if pins is not None:
                    bit = pins[self.DATA_IDX]
                    idle_val = 1 if not self._inverse_convention else 0
                    if bit == idle_val:
                        self._glitch_rejects += 1
                        self.log("start-bit glitch rejected (#%d)"
                                 % self._glitch_rejects)
                        continue
    ...
    def decode_step(self):
        ...
        # event emission, no advancement).  The reset() defaults are both 0,
        # which is indistinguishable from a card that is genuinely in reset;
        # a capture started during power-on (RST already LOW, the assertion
        # edge in the past) would otherwise miss the assertion and try to
        # measure the ETU while the card is inactive.  With the true initial
        # levels, RST-low-at-start means "expect a cold-boot ATR on deassert"
        # and the DATA state waits for that deassert before measuring.
        # Re-anchor the hysteresis candidates relative to the primed level so
        # the first confirmed transition is measured from the true state.
        if not self._primed:
            self._primed = True
            pins = self.wait({'skip': 0})
            if pins is not None:
                if self.track_rst:
                    self.last_rst = pins[self.RST_IDX]
                if self.track_vcc:
                    self.last_vcc = pins[self.VCC_IDX]
                self.rst_cand = None
                self.rst_cand_sn = 0
                self.vcc_cand = None
                self.vcc_cand_sn = 0
        if (self.track_rst and self.last_rst == 0 and self.state == 'DATA'
                    and self.bit_samples is None):
            # Card is sensed in reset at capture start (RST primed LOW;
            # the assertion edge is in the past -- e.g. the capture began
            # during power-on).  The card is inactive, so measuring the
            # ETU now would lock a bogus period from pre-ATR power-up
            # transitions on DATA.  Expect the fresh ATR instead: re-arm
            # the hunt at the spec-default rate and let FIND START wait
            # for the card to power on (read_first_byte/wait_data_falling
            # service RST/VCC events while DATA is idle).  This is the
            # "a mid-session capture may still begin with a reset" case.
            self.state = 'FIND START'
            self._atr_hunt_count = 0
            self._pps_attempted = False
            self.bit_samples = None
            ...
            self._clock_skip_confident = \
                not (self.sample_as_clock or self.detect_clock)
            self.log("RST low at start: expecting cold-boot ATR")
        ...
        if self.state == 'FIND START':
            ...
            if self.bit_samples is None or self._pps_speed_changed:
                self._pps_speed_changed = False
            ...
            if (firstByte == 0xFF): # PPS Request
                ...
                # garbage APDUs).
                self._note_discard(firstByte, 'idle-line noise (6+ bits set)', idle=True)
                self._consume_first()
                self._resync = True
                return True
                ...
                self._note_discard(firstByte, 'suspicious CLA (not ISO 7816-4)')
                self._consume_first()
                ...
                self._note_discard(firstByte, 'stuck-low noise (zero_run >= 32)', idle=True)
                self._consume_first()
                self._resync = True
                return True
            ...
            if (self.hasT0):
                ...
                if first is None:
                    return True
                if first == idle_byte:
                    self._note_discard(first, 'idle byte during header wait', idle=True)
                    self._consume_first()
                    self._resync = True
                    return True
                ...
                if (bIns == 0x00
                        ...
                        or (bIns & 0xF0) == 0x90
                        or (bIns & 0x01) != 0):
                    # Byte-slip re-frame: the whole 5-byte window is a valid
                    # size but the INS is unusable, so the window is
                    # probably shifted by one (a stray leading byte from the
                    # previous exchange, e.g. a leftover SW2).  Drop ONLY the
                    # leading byte and re-frame the remaining four so a real
                    # command that would otherwise be swept into a garbage
                    # header (or swallowed by a bogus P3) is recovered.
                    self._note_discard(bClass, 'invalid INS 0x%02x in header; re-framing'
                                       % bIns, idle=False)
                    self.peeked_samplenum = es
                    self.peeked_byte = None
                    self._replay.appendleft(p3)
                    self._replay.appendleft(p2)
                    self._replay.appendleft(p1)
                    self._replay.appendleft(bIns)
                    return True
                ...
                if self._edge_read:
                    ...
                    if pb0 is None:
                        return True
                    # Skip NULL (0x60) procedure bytes and turnaround idle
                    # bytes (0xFF): the card may send NULLs while processing
                    # before the ACK, or the bus may idle at xFF during
                    # turnaround on the bidirectional DATA line.  Without
                    # this the NULLs / idle bytes are mis-identified as
                    # case-3/4 command data.
                    for _ in range(MAX_TPDU_LEN):
                        if pb0 != 0x60 and pb0 != 0xFF:
                            break
                        self._note_discard(pb0,
                                           'idle/NULL byte before procedure byte',
                                           idle=True)
                        pb0 = self.read_byte()
                        if pb0 is None:
                            return True
                        ...
                        if (p3 == 0):
                            ...
                            for _proc in range(32):
                                ...
                                if (pb == bIns or pb == (bIns ^ 0xFF)):
                                    ...
                                    packet.append(b)
                                    continue
                                elif (pb == 0x60 or pb == 0xFF):
                                    # NULL (0x60): card busy.  Turnaround
                                    # idle (0xFF): bidirectional bus turnaround
                                    # between terminal command data and the
                                    # card's procedure byte.  Both: consume and
                                    # wait for next procedure byte.
                                    continue
            ...
            if (len(packet) > 0):
                ...
                if (all(b == 0 for b in packet)):
                    ...
                    emb = embedded_exchanges(packet)
                    if emb:
                        # CONCAT: a byte-drop mis-frame swallowed several
                        # real exchanges into this structurally valid
                        # packet (their headers survive in the data region).
                        # Flag BAD_FCS + log so downstream tools can
                        # split/reject the wrong-APDU group.
                        self.put(es, self.samplenum, self.out_ann,
                                 [Ann.ANN_WARN,
                                  ["CONCAT APDU (embedded exchange(s))"]])
                        self.log("CONCAT APDU: %d embedded exchange(s) at %r; packet %s"
                                 % (len(emb),
                                    [(s, e) for s, e in emb],
                                    codecs.encode(bytes(packet), 'hex')))
                        self.emit_packet(GSMTAP_SIM_APDU, bytes(packet),
                                         es, self.samplenum,
                                         flags=GSMTAP_FLAG_BAD_FCS)
                    else:
