...
def selftest():
    ...
    arr = np.zeros(n, dtype=np.uint8)
    t = np.arange(n) / sr
    arr |= ((((t * 2e6).astype(np.int64)) % 2)).astype(np.uint8) << 5   # CLK D5
    d = np.ones(n, dtype=np.uint8)
    burst = (t >= 0.15) & (t < 0.30)
    d[burst] = (((t[burst] * 1e5).astype(np.int64)) % 2)
    arr |= d << 0                                                        # DATA D0
    arr |= (((t >= 0.10) & (t < 0.15)).astype(np.uint8)) << 4            # RST D4
    arr |= ((t >= 0.05).astype(np.uint8)) << 2                           # VCC D2
    # two floating pins: thin blips scattered across the whole window
    for ch, offs in ((1, [0.02, 0.09, 0.21, 0.33, 0.41, 0.47]),
    ...
    want = {'CLK': 'D5', 'DATA': 'D0', 'RST': 'D4', 'VCC': 'D2'}
    ok = all(got.get(k) == want[k] for k in want)
