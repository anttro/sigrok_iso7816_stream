...
if diff -u "$BASELINE_CLEAN" "$CURRENT_CLEAN"; then
    ...
    echo "  - GARBAGE count must be 0 for test_8"
    echo "  - RESULT must be OK for test_8"
    echo "  - ETU values must match for all traces"
    ...
    echo "  - CONCAT count must be 0 for test_8 / test_7 / 4gmodem"
